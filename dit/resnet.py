...
class ResNet(nn.Module):
    ...
    def forward(self, x, return_feat=False, return_mid_feat=False):
        out = F.relu(self.bn1(self.conv1(x)))
        out = self.layer1(out)
        out = self.layer2(out)
        out = self.layer3(out)
        ...
        if return_feat:
            return out, feat
        elif return_mid_feat:
            return out, mid_feat
        else:
            return out


def ResNet18(num_channels=3, num_classes=10, num_blocks=[2, 2, 2, 2]):
...
def ResNet34(num_channels=3, num_classes=10):
    return ResNet(BasicBlock, [3, 4, 6, 3], num_channels=num_channels, num_classes=num_classes)
